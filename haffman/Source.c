...
struct Table* read_table_of_coded_file(FILE* Coded_file, unsigned long* n) {
 ...
 void write_coded_file(FILE* Original_file, unsigned long lenght_table, struct Table** table) {
     FILE* output = NULL;
     char buf;
     char* coded_byts;
     ...
     while (fread(&buf, sizeof(char), 1, Original_file) == 1) {
         count_byts = coder(buf, &coded_byts, 0);
     }
     if (feof(F)) {
         count_byts = coder(buf, &coded_byts, 1);
     }
     else {
  ...
  write_decoded_file(FILE* Input_file /*, указатель на начало данных в закодированном файле*/){
      FILE* output = NULL;
      char buf;
